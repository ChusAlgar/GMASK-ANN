...
def GMASK(config):
    ...
    metrica = config.get('test', 'distance')  # Possible values: euclidean, manhattan, chebyshev
    ...
    algorithm = config.get('method', 'algorithm') # Possible values kmedoids, others to be defined
    ...
    logging.info("")
    logging.info('---- GMASK Parameters - tam_grupo=%s - n_centroids=%s - radius=%s - algorithm=%s - implementation=%s ----', tam_grupo, n_centroides, radio, algorithm, implementation)
    ...
    n_capas, grupos_capa, puntos_capa, labels_capa = knn.gmask_tree(cant_ptos, tam_grupo, n_centroides,
                                                                     metrica, vector_training, dimensionalidad, algorithm, implementation)

    # Store index in a file
    # with open("./other_algorithms/GMASK_MNIST" + str(k_vecinos) +".pickle", 'wb') as handle:
    ...
    for i in range(len(vector_testing)):
        ...
        vecinos_i = knn.gmask_radius_search(n_centroides, punto, vector_training, k_vecinos, metrica,
