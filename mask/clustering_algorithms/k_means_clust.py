...
def centroid_pairwise_dist(x, centroids, metric):
    # SKLEARN - pairwisedisntances
    # distances = pairwise_distances(x, centroids, metric=metric)

    if metric == 'manhattan':  metric = 'cityblock'  # SCIPY - cdist (necesario traducir manhattan como cityblock)
...
def assign_clusters(data, centroids, metric):
    ...
    distances_from_centroids = centroid_pairwise_dist(data, centroids, metric)
...
def compute_heterogeneity(data, k, centroids, cluster_assignment, metric):
    heterogeneity = 0.0
    for i in range(k):
        # Select all data points that belong to cluster i. Fill in the blank (RHS only)
        member_data_points = data[cluster_assignment == i, :]
        ...
        if member_data_points.shape[0] > 0:  # check if i-th cluster is non-empty
            ...
            distances = pairwise_distances(
                member_data_points, [centroids[i]], metric=metric
            )
...
def kmeans(data, k, initial_centroids, maxiter=500, record_heterogeneity=None, verbose=False, metric='euclidean'):
    ...
    for itr in range(maxiter):
        ...
        cluster_assignment = assign_clusters(data, centroids, metric)
        ...
        if record_heterogeneity is not None:
            # YOUR CODE HERE
            score = compute_heterogeneity(data, k, centroids, cluster_assignment, metric)
